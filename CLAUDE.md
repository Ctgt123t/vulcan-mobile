...
| `server/index.js` `runStructuredAssessment` | Shared assessment machinery (2C-3): recall/TSB + DTC enrichment + fail-soft spec injection + endpoint-specific extra blocks + snapshot block LAST + forced tool call + cost log (by callType) + `softValidateAssessmentPlan`. Used by both assess + evidence-update. |
