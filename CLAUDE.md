...
- **Deferred (NOT done in this wiring pass):** Batch B fuzzy/normalized dedup — near-dup hit lines (e.g. "Oil type: dexos" + "Oil type: dexos specification") and the text-phrasing conflict-log noise on the identical test sources (both log/cosmetic, absent in production where a vehicle has one source). And **intent-widening** for the `spec_type`s Batch A added but `detectSpecIntent` can't yet request (fuel/axle/transfer-case/gvwr/gawr/idle/spark-gap/refrigerant/tire). **Known hole until that widening lands:** "spark plug gap" / "refrigerant" questions still route nowhere and reach Claude **unguarded** (they match `isSpecShapedQuestion` for cache-exclusion but not `detectSpecIntent`, so no DB lookup and no preamble) — the widening pass closes it by both routing to the DB and applying the guardrail.
