...
- **Selectable live PID monitoring** (`app/obd2.tsx` + `app/obd2-pids.tsx`) — the OBD2 screen's live data view used to poll a hardcoded 9-PID round-robin; it now polls only the PIDs the technician has selected. Selection screen at `/obd2-pids` groups PIDs by category (Engine / Fuel System / Air-Intake / Oxygen Sensors / Emissions / Speed-Transmission / Electrical / Other) — categories are assigned server-side in `pidDatabase.js:categorizeSignal` so all clients render the same buckets. Saved presets live in AsyncStorage cross-vehicle (`vulcan:pids:presets`); per-vehicle selected codes + known-unsupported codes also in AsyncStorage. **Scaling:** all preference storage is per-device, no backend per-user state added
- **PID polling driver** (`lib/obd2.ts`) — `startPolling(selectedPids, options)` drives the tick loop. Mode 01 PIDs are batched up to 6 per request using ELM327 multi-PID syntax (`01 0C 0D 0E 11`) with interleaved `41 XX` response parsing in `parseMultiPidResponse`. Mode 22 manufacturer PIDs poll sequentially every Nth tick (default every 4th) since they can't share a request. Generic byte/bit decode via `decodePidGeneric` reads `decode.{length, multiplier, divisor, offset, signed, startBit}` from the OBDb signal metadata. `getSupportedMode01Pids()` queries the ECU's PID 00/20/40/60/80/A0/C0/E0 support bitmasks so the selection UI only offers PIDs the live vehicle actually responds to. PIDs that NO DATA or time out get added to an `unsupportedPids` set and skipped on future ticks. LiveData is now a `Record<code, LiveValue>` map keyed by PID code (e.g. `"01 0C"`)
