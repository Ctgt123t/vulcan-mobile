...
- **AI:** Claude Opus 4.6 for Diagnose mode, Claude Sonnet 4.6 for Ask Vulcan mode. All three Claude prompts (`ASK_SYSTEM_PROMPT`, `SYSTEM_PROMPT`/Diagnose, `ASSESS_SYSTEM_PROMPT`/Smart Diagnose) begin with a shared `APP_CONTEXT` block (`server/index.js`) so Claude reasons as an integrated diagnostic tool — aware that the app retrieves VIN/DTCs/live data through the Vulcan OBD2 connection, knows the four app modes, and treats codes appearing in conversation as confirmed scans rather than hypothetical. Updates to the shared identity belong in `APP_CONTEXT` so all modes stay consistent. **Factory-spec provenance rule lives in `APP_CONTEXT`** (so all three modes inherit it in one place): Claude reasons freely about diagnosis/component locations/procedures/how-systems-work, but may NOT state a specific numeric factory spec (capacity, torque, viscosity, pressure, sensor voltage/range, RPM target, gap, interval, fill spec) as fact unless that exact value was injected as verified data in the current context. The test is **provenance** ("was this value given to me as verified data?"), not confidence ("am I sure?") — this is the calibration fix for confidently-wrong specs and fires regardless of how certain the model feels. Smart Diagnose's `ASSESS_SYSTEM_PROMPT` keeps its own stricter, absolute `NON-NEGOTIABLE` variant (unverified specs go in the `unverified_specs_needed` schema field, no ballpark allowance); the APP_CONTEXT baseline and the assess override layer cleanly (later + more-specific wins). `SPEC_CAUTION_PREAMBLE` (`vehicleSpecs.js`) reinforces the same provenance rule for the specific no-verified-data spec path in Ask Vulcan. *(Future nice-to-have, now non-load-bearing: the `SPEC_PATTERNS` regexes in `vehicleSpecs.js` that trigger the preamble are brittle — e.g. spark-plug gap, tire pressure, refrigerant charge don't match — but the APP_CONTEXT rule covers all spec questions regardless of whether the preamble fires.)*
