...
- DTC database in `server/dtc_codes.db` (SQLite, sourced from the MIT-licensed [Wal33D/dtc-database](https://github.com/Wal33D/dtc-database)) — 18,805 entries across 33 manufacturers plus 9,415 generic SAE J2012 codes. Queried via `better-sqlite3` in `server/dtcDatabase.js`
- DTC endpoint `GET /api/dtc/:code` accepts optional `?make=Ford` to prefer a manufacturer-specific definition; falls back to the generic SAE entry, then to pattern handlers (cylinder/coil-specific copy), then 404
