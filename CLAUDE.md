...
- **Database:** Supabase (Postgres) is the backend's DB for the new unified vehicle-data layer (the spec database). Connected server-side via `pg` (node-postgres) over the **Supabase Transaction Pooler** (Supavisor, port 6543) — see DB Layer section. Foundation only right now: connection + schema exist; the extraction pipeline and the JSON-cache → Postgres data migration are both deferred. **The existing JSON caches are unchanged and remain the source of truth for their features** (`cache.json`, `dtcCache.json`, `vehicleSpecCache.json`, `pidCache.json`).
...
### Database layer (Supabase / Postgres) — foundation

The unified vehicle-data layer's storage. **Foundation only: connection + schema. No extraction pipeline yet, no data migrated from the JSON caches (that migration is deferred), and the JSON caches are untouched.**
...
- **Extraction PoC (vertical slice)** — `npm run extract:pdf` (`server/scripts/extractFromPdf.js`) proves doc → verified rows: a source PDF → `source` row → Claude (opus-4-6, forced `tool_choice` on `emit_extracted_specs`) extracts specs + component facts → validation gate → `spec`/`component_fact` with the `source_id` FK → read-back. **Core rule:** extract ONLY what the doc states; every item needs a `verbatim_quote` + page (no quote → quarantined, never stored). Manual-run only, not wired into any endpoint; does not touch the live spec path or JSON caches. Uses `pdf-lib` to split large PDFs. **Key finding from the 2011 Sierra owner's manual (594 pages):** the whole doc is **~1.185M tokens — exceeds Opus 4.6's 1M context window**, so it cannot be sent whole (the script chunks at 150 pages/chunk). Cost is **input-dominated** (~$5.95 input / $0.36 output ≈ $6.31 total for the full manual); pages 1–300 cost ~$3.00 and yielded **zero** specs (all specs live in the back half) — so trimming to the spec/maintenance sections is the main cost lever. The PoC also showed the controlled vocab is too narrow (many real specs — fuel-tank capacity, axle lubricants, GVWR weights — fall into `other`); widen `spec_type` before productionizing. The test PDF lives under the gitignored `server/extraction_test/`. The extraction pipeline beyond this slice is **not built**.
...
- Vehicle spec retrieval in `server/vehicleSpecs.js` — `detectSpecIntent`/`SPEC_PATTERNS` pattern-detect factual spec questions (oil capacity, torque, fluids, battery, maintenance intervals), and `lookupSpec(vehicle, specType)` routes through a provider chain. Provider modules live in `server/specProviders/` and follow a `{ id, configured(), lookup(vehicle, specType, params, fetcher) }` shape. **The sole spec source is now the Supabase vehicle-data layer (`supabaseSpecs.js`). Vehicle Finder was removed — proven a dead end (3 spec types, sparse, silently-wrong records).** The provider shape is kept so a future Tier-2 retrieval-grounded provider can slot in.
  - **Ask Vulcan (`/api/ask`) routes spec intent via TOOL USE, not the regex.** The brittle `detectSpecIntent` adjacency regex missed phrasings like "oil change specs" / "what oil does it take" (it caused the 2026-06 hedged-answer-despite-correct-data bug on a 2020 F-150). Spec routing now lives in `server/askToolLoop.js`: an agentic execute-then-continue tool loop registers a `spec_lookup` tool (input = `spec_types` enum only; the vehicle is injected server-side from request context, not a tool param), Claude calls it on any spec question — including indirect phrasings — and reasons over the verified rows in the same turn. **Hybrid seam:** `detectSpecIntent` is kept ONLY as a zero-Claude latency fast-path (regex fires + structured vehicle + DB hit → formatted card, no Claude call); every other case (no regex match, regex-hit + DB-miss, or no vehicle) flows to the tool loop. **Both doors call the identical `lookupSpec` — one source of truth.** The loop has an env-configurable iteration cap (`ASK_TOOL_MAX_ITERATIONS`, default 3) plus a forced text-only final call so it can't hang, sums cost across every call into one `ApiCostData` object, and reports `toolInvoked` so a tool-using response is never cached (live data). Adding a future tool (e.g. fuse diagram) is "register one entry" in `ASK_TOOLS` + `ASK_TOOL_HANDLERS` — the loop is the shared groundwork the Stage 2 monitoring loop also needs. **Diagnose/Assess do NOT use the tool loop** — they keep their forced-tool calls and proactive `detectAllSpecIntents` spec-context injection (a separate later decision).
