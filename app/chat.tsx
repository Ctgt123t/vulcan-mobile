...
import {
  ...
  upsertLightThread,
} from "../lib/lightThreads";
import { LIGHT_SCHEMA_VERSION, makeThreadId } from "../lib/lightThreadsCore";
...
// Phase 4 (unified shell): ONE chat screen, three phases. "light" is the
// casual Ask channel (the /api/ask brain — cheap, no VIN required, zero-cost
// fast-paths intact); "intake" is the escalation beat (vehicle + mileage +
// complaint — the single visible, billable event; the Phase-2 diagnosis
// credit mints at submit); "chat" is the structured diagnostic engine,
// UNCHANGED. Once escalated a thread stays diagnostic (locked decision §1.9)
// — there is no path from "chat" back to "light"; a fresh casual question is
// a new thread.
type Phase = "light" | "intake" | "chat";
...
export default function Screen() {
  ...
  // Entry modes (unified shell): default = "light" (the merged product — one
  // chat that starts casual). ?mode=diagnose → straight to intake (the home
  // Diagnose tile + OBD2 escalation). ?resume=<caseId> → intake until the
  // resume applies (as before). ?thread=<lightId> → light, restored by the
  // thread effect below.
  ...
  const [phase, setPhase] = useState<Phase>(() =>
    entryParams.mode === "diagnose" || typeof entryParams.resume === "string"
      ? "intake"
      : "light",
  );
  // Captured once: whether this screen instance ENTERED at the diagnostic
  // intake (gates the mount-time intake-consume effect — a light entry must
  // not drain escalation handoffs or clear the global vehicle).
  const enteredAtIntakeRef = useRef(
    entryParams.mode === "diagnose" || typeof entryParams.resume === "string",
  );
  ...
  // ---- Phase 4: the light channel (Ask) inside the shell ----
  // The persistent light-thread identity (Phase 3 store). null until the first
  // send persists the thread. On escalation the thread's content moves into
  // the diagnostic case envelope and the light record is deleted (no double
  // entry in the chats list); escalatedFromLightRef carries the id across the
  // intake beat so the delete happens only once the case actually exists.
  const lightThreadIdRef = useRef<string | null>(null);
  const lightCreatedAtRef = useRef<string | null>(null);
  const escalatedFromLightRef = useRef<string | null>(null);
  ...
  // Usable vehicle context for the Ask API (any year/make/model triple).
  const hasLightVehicle = Boolean(
    vehicle.year?.trim() && vehicle.make?.trim() && vehicle.model?.trim(),
  );
  const [vehicleModalOpen, setVehicleModalOpen] = useState(false);

  // ?thread=<id> — resume a persisted light thread (the chats list). Restores
  // the messages + the thread's vehicle (only when DISCONNECTED — a live
  // connection's vehicle is ground truth and is never overwritten; Ask has
  // always answered against the global vehicle).
  useEffect(() => {
    const threadId =
      typeof entryParams.thread === "string" ? entryParams.thread : null;
    if (!threadId) return;
    let active = true;
    (async () => {
      const t = await loadLightThread(threadId);
      if (!active) return;
      if (!t) {
        Alert.alert(
          "Chat unavailable",
          "This conversation couldn't be loaded — it may have been removed, or was saved by a newer version of the app.",
        );
        return; // stays a fresh light chat
      }
      lightThreadIdRef.current = t.id;
      lightCreatedAtRef.current = t.createdAt;
      setMessages(t.messages);
      messagesRef.current = t.messages;
      setPhase("light");
      if (t.vehicle && !isConnected) {
        setVehicleManually({ ...EMPTY_VEHICLE, ...t.vehicle }, t.vin).catch(
          () => {},
        );
      }
    })();
  ...
  // Persist the light thread (Phase 3 store). Fail-soft BY CONTRACT — a
  // storage error must never break the chat. Strips diagrams (Brave ToS:
  // never stored) and base64 (lean rule) at WRITE time; the migrator would
  // strip them on load anyway, but they must not touch disk at all.
  async function persistLightThread(msgs: ChatMessage[]): Promise<void> {
    try {
      if (!lightThreadIdRef.current) {
        await pruneForNewLightThread();
        lightThreadIdRef.current = makeThreadId();
        lightCreatedAtRef.current = new Date().toISOString();
      }
      const clean: ChatMessage[] = msgs.map((m) =>
        m.image
          ? { role: m.role, content: m.content, image: withoutBase64(m.image) }
          : { role: m.role, content: m.content },
      );
      await upsertLightThread({
        schemaVersion: LIGHT_SCHEMA_VERSION,
        id: lightThreadIdRef.current,
        vehicle: hasLightVehicle ? vehicle : null,
        vin: vin.trim() || null,
        messages: clean,
        createdAt: lightCreatedAtRef.current ?? new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      });
    } catch {
      // fail-soft: the in-memory thread is intact; the next save retries
    }
  }

  // One light turn: user message → /api/ask → plain-text assistant reply
  // (+ diagrams, in-memory only). Ported from app/ask.tsx send() onto the
  // shell's shared state; persisted per completed turn (user turn saved
  // BEFORE the call so a crash can't lose input — the case-store discipline).
  async function sendLightMessage(text: string, image?: ImageAttachment | null) {
    ...
    const stored: ChatMessage = image
      ...
      : { role: "user", content };
    const next: ChatMessage[] = [...messagesRef.current, stored];
    setMessages(next);
    ...
    setLoading(true);
    setError(null);
    void persistLightThread(next);
    // Lean history: inject the transient base64 into the LAST user turn (the
    // attach turn) of a local clone — sent on THIS ask only.
    let outgoing = next;
    ...
    try {
      const reply = await ask(
        outgoing,
        hasLightVehicle ? vehicle : undefined,
        recalls,
        tsbs,
      ...
      void persistLightThread(appended);
      if (reply.cost) {
        diagnosticLogger.log({
          type: "ask_vulcan",
          vehicle: hasLightVehicle
            ? {
                year: vehicle.year,
                make: vehicle.make,
                model: vehicle.model,
  ...
  // THE ESCALATION (locked decisions §1.7–1.9): the always-available manual
  // "Diagnose this" action. Carries the whole light thread in memory into the
  // intake beat (the Phase-1 seeding machinery — incl. the carried-photo
  // re-attach — runs unchanged at submit); the complaint prefills from the
  // last user message; the Phase-2 credit mints at submit with source "ask".
  function escalateToDiagnosis(): void {
    const lastUser = [...messagesRef.current]
      .reverse()
      .find((m) => m.role === "user");
    const carried: ChatMessage[] = messagesRef.current.map((m) =>
      m.image
        ? { role: m.role, content: m.content, image: withoutBase64(m.image) }
        : { role: m.role, content: m.content },
    );
    ...
    escalatedFromLightRef.current = lightThreadIdRef.current;
    setSymptom(lastUser?.content ?? "");
    setError(null);
    setPendingPhoto(null);
    setPhase("intake");
  }
  ...
  function startNewLightChat(): void {
    ...
    lightThreadIdRef.current = null;
    lightCreatedAtRef.current = null;
    escalatedFromLightRef.current = null;
  }
  ...
  async function onSubmitIntake() {
    ...
    // Phase 4: an in-shell escalation moves the light thread's content into
    // the case envelope (the carried-thread seeding below) — delete the light
    // record so the chats list shows ONE entry (the case), not two. Only when
    // a case actually exists (an unsaved-at-cap session keeps the light
    // record as the sole survivor of the conversation).
    if (escalatedFromLightRef.current && caseMetaRef.current) {
      deleteLightThread(escalatedFromLightRef.current).catch(() => {});
    }
    escalatedFromLightRef.current = null;
    lightThreadIdRef.current = null;
    lightCreatedAtRef.current = null;
  ...
  // (Phase 4: onSwitchToAsk is GONE — locked decision §1.9, an escalated
  // thread stays diagnostic. A fresh casual question is a new thread from the
  // chats list; casual side-questions inside a diagnosis arrive with Phase-5
  // in-diagnosis retrieval. The to_ask handoff has no writer anymore.)
  ...
  function resetSession() {
    ...
    // Phase 4: reset also drops any light-thread identity — a "New Diagnosis"
    // from diagnostic mode starts clean.
    lightThreadIdRef.current = null;
    lightCreatedAtRef.current = null;
    escalatedFromLightRef.current = null;
      ...
      >
        <FlatList
          ...
          contentContainerStyle={[
            ...
            threadRows.length === 0 && styles.threadContentEmpty,
          ]}
          data={threadRows}
            ...
            phase === "light" ? (
              <View style={styles.emptyWrap}>
                ...
                <Text style={styles.emptyBody}>
                  Ask anything automotive — specs, procedures, how systems
                  work — or describe a problem and escalate to a full
                  diagnosis any time.
                </Text>
              </View>
            ) : null
        ...
        <SafeAreaView edges={["bottom"]} style={styles.footerBar}>
          {confirmedDone ? (
            ...
            <>
              {phase === "light" && (
                // THE manual escalation (locked §1.7): always available in the
                // light phase. The brain may also gently OFFER escalation in
                // prose (the ASK_SYSTEM_PROMPT invitation — never forced); the
                // tap is the single visible, billable event's doorway.
                <TouchableOpacity
                  style={styles.escalateBtn}
                  onPress={escalateToDiagnosis}
                  activeOpacity={0.75}
                  accessibilityRole="button"
                  accessibilityLabel="Diagnose this — run a full structured diagnosis"
                >
                  <Ionicons
                    name="pulse-outline"
                    size={15}
                  ...
                  />
                  <Text style={styles.escalateBtnText}>Diagnose this</Text>
                </TouchableOpacity>
              )}
              <View style={styles.composerWrap}>
                ...
                <View style={styles.answerRow}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={
                      phase === "light"
                        ? () => sendLightMessage(answer, pendingPhoto)
                        : onSubmitAnswer
                    }
                    disabled={
...
function MessageRow({ message }: { message: ChatMessage }) {
  ...
  try {
    ...
    // Plain-text assistant turn — the LIGHT channel's normal reply shape
    // (and carried Ask turns inside a diagnostic case). Renders any attached
    // diagram results (in-memory only — never persisted, Brave ToS).
    return (
      <View style={styles.assistantTurn}>
        <Text style={styles.assistantLabel}>VULCAN</Text>
        <View style={[styles.bubble, styles.bubbleAssistant]}>
...
const styles = StyleSheet.create({
  ...
  // Phase 4 (unified shell) — light-phase chrome.
  threadContentEmpty: {
    flexGrow: 1,
  },
  emptyWrap: {
