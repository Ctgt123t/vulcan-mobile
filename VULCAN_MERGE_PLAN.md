...
- **Phase 1 BUILT (2026-07-01):** full-context escalation implemented mobile-only (see §7 row 1); all local gates pass; **pending founder on-device validation, then `eas update`.**
- **Next:** founder validates Phase 1 on-device → ship OTA → begin **Phase 2** (metering foundation). Do **not** start any server/prompt work beyond Phase 2's additive metering until Phases 1–4 contracts are settled; Phase 5 is the only fence-adjacent change and is gated last. **The unified shell (Phase 4) must not ship before persistent chats (Phase 3) — hard gate.**
