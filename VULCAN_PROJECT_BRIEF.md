...
> **Last updated: 2026-06-28.** Since the 06-21 UI completion, two big things landed. **(1) The unified data layer's real spec feed RAN and is closed** — what the 06-21 brief called "DEFERRED to near-launch" is done: run-1 (90 vehicles, ~$202) + a run-2 OEM-portal top-up (13 vehicles, ~$32) + a spec-propagation engine = **396 answerable year-make-model entries (106 real-extracted + 290 propagated-inferred)** in the Supabase spec DB, plus a vocabulary cleanup (the `other` bucket worked down) and NHTSA-canonical normalization (`nhtsa_make` populated). **(2) In-app diagram lookup shipped** (Brave Image Search — fuse/component/wiring diagrams for the in-context vehicle, no-fabrication, "Powered by Brave"). A handful of diagnostic-engine fixes also shipped (capture-trigger, recalls advisory-only, freeze-frame guard — prompt-only, not yet re-validated on the real CR-V). **The one remaining major track is still Phase-4 pre-launch infrastructure** (auth, billing, analytics, a dedicated Brave production key, self-hosted vPIC, structured-input pickers). See "Recently shipped," the data-layer section, and the phase tracker. Full data-layer detail: `VULCAN_DATA_LAYER_STRATEGY.md` §11.
>
> _(Prior, 2026-06-21: the entire v2 "steel glass" UI redesign shipped across every screen; the diagnostic engine was already complete + hardware-validated and the flow refined against real shop testing.)_
...
## The app modes
1. **Ask Vulcan** — open-ended Q&A, no VIN required, conversational. Pulls TSBs/recalls/specs. **Supports photo evidence** (attach a photo to a question; image-bearing asks bypass the response cache). **In-app diagram lookup** (fuse/component/wiring images via Brave Image Search) is exposed here as a `diagram_lookup` tool (and as a "Find a diagram" modal mid-diagnosis).
...
## THE UNIFIED DATA LAYER (Phase 4 — foundation + extraction engine built; the real feed RAN [396 answerable YMM live]; ongoing = demand-ranked top-ups + crowdsource for the verifiability walls)
...
**Why it exists:** Vehicle Finder API was proven a dead end (covers only 3 spec types, sparse, returns wrong records silently). The fix is a proprietary database Vulcan owns. Five data types: DTC defs (✅ done, 18,805 codes), VIN decode (✅ mostly, NHTSA), PID defs (✅ done, OBDb), **service specs (✅ feed ran — 396 answerable YMM, extracted + propagated)**, **component/config facts (✅ extracted alongside specs — ~9,268 component facts live)**.
...
**Tier 1 feed — ✅ RAN & CLOSED (2026-06-27/28, no longer deferred).** The real extraction feed executed end-to-end at scale: **run-1** (90 vehicles, ~$202, sourced from the dealereprocess dealer-platform CDN, identity guard-gated text→vision→skip, never `--identity-override`), a **vocabulary cleanup** (the `other` bucket re-keyed down via Batch D/E + a textual quote-parser, all reversible), a **spec-propagation engine** (fills missing same-generation/same-engine vehicles' STABLE physical specs from extracted anchors for ~$0 — allowlist-only, hidden `origin` extracted/inferred marker, ±2-year reach, no-clobber, fluid-spec strings NEVER propagated), and a **run-2 $50 OEM-portal top-up** (13 new anchors: Honda CR-V via techinfo, Ford Super Duty, clean un-anchored GM/Nissan/Mazda/Jeep gens). **Result: 396 answerable year-make-model entries = 106 real-extracted + 290 propagated-inferred** (~73% propagated for ~$0; ~$232 cumulative Claude extraction spend). **The verifiability finding:** Toyota RAV4/Tacoma/Tundra/Avalon, all Hyundai, all Kia, Subaru Forester/Crosstrek ship inherently-generic owner's manuals (model name absent in text AND cover) → unverifiable under the no-override floor → genuine gaps, coverable only by Tier-3 crowdsourcing or a licensed feed. Ongoing data-layer work is demand-ranked top-ups (off the `spec_miss` log once there's real traffic), not a one-time feed. (Full ledger: VULCAN_DATA_LAYER_STRATEGY.md §11.)
...
**Spec data layer — the real feed RAN & CLOSED (2026-06-27/28).** Run-1 (90 vehicles) + Batch D/E vocab cleanup + spec-propagation engine + run-2 OEM top-up (13 vehicles) = **396 answerable YMM (106 extracted + 290 propagated)**. Sourcing rule settled (any fetchable, identity-verified, quote-gated PDF is in-bounds; OEM is a tie-breaker, not a requirement; never filename-trust, never `--identity-override`). Propagation safety: inferred specs stored direct + presented "here it is" with a hidden `origin` marker, never claimed "verified" if explicitly asked, overwritten the instant a real manual for that vehicle is extracted. The high-volume verifiability walls (RAV4/Tacoma/Tundra, all Hyundai/Kia, Subaru Forester/Crosstrek — generic manuals) are honest gaps, coverable only by crowdsourcing or a licensed feed. See the data-layer section + `VULCAN_DATA_LAYER_STRATEGY.md` §11.

**In-app diagram lookup — V1 shipped to production (2026-06-27, server + OTA).** A tech pulls a real fuse-box / belt-component / wiring diagram for the in-context vehicle via **Brave Image Search** — fuse images (~92%, ships), narrow year-verified component images (~35%, the rest fall to a link), wiring = links-only; a universal "search the web" fallback so it's never a dead end. Year/generation guard (a wrong-gen fuse box is dangerous — no best-guess), no-fabrication rule, **"Powered by Brave"** attribution, transient in-memory only (Brave ToS — never stored/relabelled). Exposed as an Ask Vulcan `diagram_lookup` tool + a mid-diagnosis "Find a diagram" modal (the modal hits the endpoint directly, NOT wired into the diagnosis brain). **Running on the dev/probe Brave key as a stopgap — a dedicated production/enterprise key is a pre-launch item.**

**Diagnostic-engine fixes (2026-06-23, prompt-only, NOT yet re-validated on the real CR-V):** capture-trigger fix (`measured_targets[]`, arm-on-context-gate, per-item capture cards), recalls/TSBs reframed advisory-only (`UNIFIED_RECALL_ADVISORY_SECTION`), and a default/sentinel freeze-frame guard. (Decided NOT to ingest diagnostic *manuals* — not legitimately ingestible; diagram *retrieval* shipped instead. PID known-good reference data deferred post-launch.)
...
| 4. Pre-launch infrastructure | 🔄 IN PROGRESS (data-layer spec feed ✅ RAN — 396 answerable YMM; NHTSA normalization ✅ done; diagram lookup ✅ shipped; iOS Classic-pod cleanup ✅ done. Remaining: auth, billing, analytics, dedicated Brave production key, self-hosted vPIC, structured-input pickers, JSON→Postgres migration) |
...
- **Unified data layer** (replaces Vehicle Finder). Supabase foundation ✅; **real feed RAN ✅ — 396 answerable YMM (106 extracted + 290 propagated), NHTSA-canonical normalization done (`nhtsa_make` populated)**. Ongoing = demand-ranked top-ups off the `spec_miss` log + the crowdsource/licensed-feed path for the high-volume verifiability walls (RAV4/Tacoma/Tundra, Hyundai, Kia, Subaru Forester/Crosstrek).
- **Structured-input pickers (#14)** — mobile make/model/year pickers; `nhtsa_make` is populated and ready to feed them.
- **Dedicated Brave production/enterprise key** for diagram lookup (currently on the dev/probe key as a stopgap) — via Brave sales. **Legal pass:** Brave "display open-web thumbnails in a commercial app" posture + the licensed-data-feed question (if pursued for the unsourceable walls).
