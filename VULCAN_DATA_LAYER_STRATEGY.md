...
- **Identity is provenance-only on Subaru (#17 — FIXED).** The manual carries NO model/year in extractable text anywhere in 496 pages (no text cover; the foreword says "your SUBARU vehicle"); neither the text guard nor a vision pass can confirm identity — it rests on the manufacturer's portal serving the file under the declared vehicle. `--identity-override="<justification>"` added: failed check + justification logged loudly and persisted in the run snapshot; the no-flag abort is unchanged. Corroboration used: sedan+5-door body set, FB20 2.0L p451, 13.2-gal tank p451 — uniquely 2023 Impreza.
...
- These are gated by the **GM-shaped-assumptions standing principle** (§6.1): run a diverse manufacturer set and hunt for GM-fitting before trusting at scale.
