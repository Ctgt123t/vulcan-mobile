...
- **Vocab / unit widening (#9) — STILL DEFERRED, with Batch C / the feed.** Ford manuals park real specs in the `other` bucket that deserve their own types: **compression_ratio, displacement, firing_order, octane, DEF (diesel exhaust fluid)**. (The earlier "lb.ft"/"lbf·ft" unit-parsing gap was already closed by `normUnitForMatch` in the Subaru run.) Widen the vocab alongside the Batch C schema expansion / first feed.
