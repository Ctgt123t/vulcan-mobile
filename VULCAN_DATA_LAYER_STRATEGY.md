...
>
> **CURRENT POSTURE — PAUSED IN A KNOWN-GOOD STATE TO MOVE TO STAGE 2 (deliberate, not abandoned).** The data layer is at a clean resting point and work is intentionally shifting to the diagnostic engine's Stage 2. What is DONE: foundation live; extraction engine productionized (Batch A) **and validated cross-manufacturer** (run on GMC Sierra + Ford + Honda owner's manuals — see §6.1); the spec path wired into the app under full Option C (verified hit / honest fallback / miss-logging, fail-soft); and **tool-use spec routing live in `/api/ask`** (the `spec_lookup` agentic loop — see CLAUDE.md → Backend → vehicle spec retrieval). What remains is **deliberately deferred and none of it blocks Stage 2** — see the deferred-work ledger (§11) for each item and its window. The two reasons this pause is safe: (1) Stage 2 reasons over OBD2 data + already-stored DB facts, so it does not depend on the deferred extraction/normalization work; (2) the pre-launch items (normalization §5.B, structured input) and the before-real-feed items (page-remap, Honda fixes) are flagged for their windows so they are not lost — "parked" here means *decided and resting*, not forgotten.
...
**Before the real feed (deferred, small, not trust-blocking):**
- **Page-remap bug (#6).** Some stored `page` numbers are wrong on Ford + Sierra rows. **Values and `verbatim_quote`s are CORRECT** — only the page index is off, so this is a citation-precision bug, not a data-trust crisis. Root cause: the prompt tells Claude to emit *absolute* manual pages, but the post-processing remap *assumes chunk-relative* and re-translates them, double-counting. **Lean fix: trust Claude's absolute page numbers and drop the remap step.** Small.
- **Honda fixes (#7).** Two issues exposed by the Honda manual: **(a) the trim-keyword dictionary is GM-shaped and SILENTLY under-selects on non-GM manuals** — this is the dangerous one (it's a quiet false-negative in page selection, not a loud error, so a Honda run can drop spec-bearing pages without complaint); **(b) `pdf-lib` copies shared page resources into every chunk**, inflating chunk size until a large manual trips the ~32MB request-size rejection. Both are before-feed gates.
- **Vocab / unit widening (#9) — with Batch C / the feed.** Ford manuals park real specs in the `other` bucket that deserve their own types: **compression_ratio, displacement, firing_order, octane, DEF (diesel exhaust fluid)**. Plus a **unit-parsing gap: "lb.ft" (with a period) isn't recognized** as the torque unit. Widen the vocab + unit normalizer alongside the Batch C schema expansion / first feed.
- **Cosmetic nit (#15) — trivial.** A stray indentation block in `server/scripts/extractFromPdf.js` (the token-preflight `else` arm). Valid JS, purely cosmetic; fold into the next extraction-pipeline change rather than spending a standalone edit.
- **Latent fuel-capacity range bug (#16) — small, found reading the validator during the Subaru run.** In `validateSpec`'s `fuel_capacity` branch, `/l|liter|litre/.test(u)` matches "ga**l**" (the letter `l`), so gallon values take the litre range (10–230) and the dedicated `/gal/` branch is unreachable. Harmless for typical tanks (13.2 gal passes either way) but a sub-10-gallon tank (kei/sport) would be wrongly quarantined. Fix by testing `/gal/` first or anchoring the litre regex.
...
- **Identity is provenance-only on Subaru (#17 — override shipped; real fix deferred to the feed window).** The manual carries NO model/year in extractable text anywhere in 496 pages (no text cover; the foreword says "your SUBARU vehicle"); the text guard cannot confirm identity — it rests on the manufacturer's portal serving the file under the declared vehicle. `--identity-override="<justification>"` added: failed check + justification logged loudly and persisted in the run snapshot; the no-flag abort is unchanged. Corroboration used: sedan+5-door body set, FB20 2.0L p451, 13.2-gal tank p451 — uniquely 2023 Impreza. **Proposed real fix (feed window): a visual cover-page check** — when the text scan fails, render/send the first page(s) to Claude as a vision pass to read the cover title and compare against the declared vehicle, so identity confirmation stays automatic for the common no-text-but-has-cover case. The override then narrows to the genuinely cover-less files (this STIS PDF has no cover at all — a vision pass finds nothing — so the override remains the documented fallback for that residue).
...
**BEFORE THE REAL FEED (small, not trust-blocking; §6.1 "Before the real feed"):**
- **#6 Page-remap bug** — some stored `page`s wrong (Ford+Sierra); values/quotes correct. Fix: trust Claude's absolute pages, drop the remap.
- **#7 Honda fixes** — (a) GM-shaped trim-keyword dictionary **silently under-selects** on non-GM manuals (the dangerous one); (b) `pdf-lib` shared-resource copying trips the ~32MB request limit on large manuals.
- **#16 Fuel-capacity gallon range unreachable** (found in the Subaru run, §6.1) — `/l|liter|litre/` matches "ga**l**" so gallon values take the litre range; a sub-10-gal tank would be wrongly quarantined. Small fix: test `/gal/` first or anchor the regex.
- **#17 Identity vision fallback** (Subaru run, §6.1) — add a visual cover-page check when the text identity scan fails, so confirmation stays automatic for no-text-but-has-cover manuals; `--identity-override` (shipped 2026-06-12, default-abort unchanged) narrows to the genuinely cover-less residue.
- These are gated by the **GM-shaped-assumptions standing principle** (§6.1): run a diverse manufacturer set and hunt for GM-fitting before trusting at scale. The Subaru run (§6.1) partially paid this down: the #9 unit-spelling normalizer is now built (`normUnitForMatch`), and a zero-cost trim preflight (`scripts/trimPreflight.js`) exists for checking #7a coverage per-manual before spending.
...
**BATCH C — schema expansion (#12; the genuinely-new scope; §6.1 SCOPE DECISION):** fuses/relays + amperage, bulbs, warning-light/DIC meanings, tire/TPMS/door-label pressures, towing/GVWR/GAWR, axle fluids, battery, fuel/octane. **This is where a future retrieval tool (e.g. a fuse-diagram lookup) registers into the PR-1 tool-loop (#3) — "register one entry," not a rewrite.** Vocab/unit widening (#9: compression_ratio/displacement/firing_order/octane/DEF + the "lb.ft"-with-period unit gap) lands with this / the feed.

**PRE-LAUNCH (hard gate, but parked behind Stage 2):**
- **#4 NHTSA-canonical normalization** (§5.B) — normalize user input + extraction-write + lookups to NHTSA's spelling so misspelled/non-canonical entry can't false-miss data that exists. Safe to defer past Stage 2 (Stage 2 doesn't depend on manual-entry resolution); must land before launch (real users will hit false misses).
- **#14 Structured input / pickers** — constrain make/model/year at the UI so free-text drift never enters. Same decision and window as #4; they ship together.
...
**AFTER STAGE 2 (Stage 2 informs the design):**
- **#10 PR-2 — Diagnose/Assess tool-use integration.** `/api/diagnose` and `/api/assess` keep their current forced-tool + proactive `detectAllSpecIntents` injection for now. Folding them onto the PR-1 tool-loop pattern is deferred **until Stage 2's evidence loop exists**, because Stage 2 will shape what a unified Diagnose tool-loop should look like (data-capture requests + spec lookups through one loop). Decided to wait, not skip. (Code-side note also in CLAUDE.md → Backend → vehicle spec retrieval.)
