...
// affordance). Search-engine posture: real open-web thumbnails, each tapping
// through to its SOURCE, a conspicuous "Powered by Brave" attribution (ToS),
// and an always-present "search the web" fallback so nothing dead-ends. We never
// claim these as Vulcan data and never describe their contents (that's the
// model's job elsewhere — and it's forbidden from doing so).
...
function Thumb({ d }: { d: DiagramResult }) {
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={`Open diagram source: ${d.title || d.domain}`}
    >
      {failed ? (
